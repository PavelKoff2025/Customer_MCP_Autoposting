...
- **Генерация изображения для поста AI** — автосоздание обложки/картинки под текст поста (промпт из контента → картинка → сразу в `create_post` / `publish_all`), без ручной подстановки `image_path`.
